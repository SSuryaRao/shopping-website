...
type LoginMethod = 'mobile' | 'userid' | 'email';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (loginMethod === 'mobile') {
        await signInWithMobile(formData.mobile, formData.password);
      ...
      // Don't redirect if account selection is needed
      // The accountOptions state will trigger the AccountSelector to show
      if (!accountOptions) {
        router.push('/dashboard');
      }
    } catch (error: unknown) {
  ...
  const handleAccountSelect = async (uniqueUserId: string) => {
    try {
      await selectAccount(uniqueUserId);
      router.push('/dashboard');
    } catch (error: unknown) {
  ...
  // Show account selector if multiple accounts are found
  if (accountOptions) {
    return (
      <AccountSelector
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Sign in to your account to start earning points
          </p>
        </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      className="appearance-none relative block w-full pl-10 px-3 py-3 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm"
                      placeholder="User ID (e.g., USR123456)"
                    />
                  </div>
                </div>
