...
function SignUpForm() {
  ...
  useEffect(() => {
    if (user && !authLoading && !needsProfileCreation) {
      router.push('/dashboard');
    }
  }, [user, authLoading, needsProfileCreation, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auth context will redirect to profile creation
    } catch (error: unknown) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Auth context will handle redirect (to profile creation or dashboard)
    } catch (error: unknown) {
        ...
        <div className="mt-8 space-y-6">
          ...
          <div className="text-center">
            <span className="text-sm text-gray-600">
              ...
              <Link href="/login-new" className="font-medium text-indigo-600 hover:text-indigo-500">
                Sign in
              </Link>
            </span>
          </div>
