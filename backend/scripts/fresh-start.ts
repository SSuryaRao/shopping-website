...
async function freshStart() {
  try {
    ...
    // Ask about products
    const keepProductsAnswer = await question('Do you want to KEEP products? (yes/no): ');
    const keepProducts = keepProductsAnswer.toLowerCase().trim() === 'yes';
    ...
    // Final confirmation
    const confirmAnswer = await question('\n⚠️  Type "DELETE ALL" to confirm (case-sensitive): ');
    ...
    if (!keepProducts) {
      console.log(`   ❌ Products:           ${await Product.countDocuments()} (deleted)`);
    } else {
